...
var UserRels = struct {
	OwnerUserGames string
}{
	OwnerUserGames: "OwnerUserGames",
}

// userR is where relationships are stored.
type userR struct {
	OwnerUserGames GameSlice `boil:"OwnerUserGames" json:"OwnerUserGames" toml:"OwnerUserGames" yaml:"OwnerUserGames"`
}
...
func (r *userR) GetOwnerUserGames() GameSlice {
	if r == nil {
		return nil
	}
	return r.OwnerUserGames
}
...
var (
	userAllColumns            = []string{"id", "username"}
...
// OwnerUserGames retrieves all the game's Games with an executor via owner_user_id column.
func (o *User) OwnerUserGames(mods ...qm.QueryMod) gameQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"games\".\"owner_user_id\"=?", o.ID),
	)

	return Games(queryMods...)
}

// LoadOwnerUserGames allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadOwnerUserGames(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`games`),
		qm.WhereIn(`games.owner_user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.OwnerUserGames = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.OwnerUserID) {
				local.R.OwnerUserGames = append(local.R.OwnerUserGames, foreign)
				if foreign.R == nil {
					foreign.R = &gameR{}
				}
...
// AddOwnerUserGames adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.OwnerUserGames.
// Sets related.R.OwnerUser appropriately.
func (o *User) AddOwnerUserGames(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Game) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			OwnerUserGames: related,
		}
	} else {
		o.R.OwnerUserGames = append(o.R.OwnerUserGames, related...)
	}
...
// SetOwnerUserGames removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.OwnerUser's OwnerUserGames accordingly.
// Replaces o.R.OwnerUserGames with related.
// Sets related.R.OwnerUser's OwnerUserGames accordingly.
func (o *User) SetOwnerUserGames(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Game) error {
	query := "update \"games\" set \"owner_user_id\" = null where \"owner_user_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.OwnerUserGames {
		...
		o.R.OwnerUserGames = nil
	}

	return o.AddOwnerUserGames(ctx, exec, insert, related...)
}

// RemoveOwnerUserGames relationships from objects passed in.
// Removes related items from R.OwnerUserGames (uses pointer comparison, removal does not keep order)
// Sets related.R.OwnerUser.
func (o *User) RemoveOwnerUserGames(ctx context.Context, exec boil.ContextExecutor, related ...*Game) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.OwnerUserGames {
			if rel != ri {
				continue
			}

			ln := len(o.R.OwnerUserGames)
			if ln > 1 && i < ln-1 {
				o.R.OwnerUserGames[i] = o.R.OwnerUserGames[ln-1]
			}
			o.R.OwnerUserGames = o.R.OwnerUserGames[:ln-1]
			break
		}
	}
