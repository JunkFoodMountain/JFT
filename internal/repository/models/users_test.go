...
func testUserToManyOwnerUserGames(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.OwnerUserGames().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadOwnerUserGames(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerUserGames); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OwnerUserGames = nil
	if err = a.L.LoadOwnerUserGames(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerUserGames); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpOwnerUserGames(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddOwnerUserGames(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.OwnerUserGames[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OwnerUserGames[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OwnerUserGames().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpOwnerUserGames(t *testing.T) {
	var err error
	...
	err = a.SetOwnerUserGames(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetOwnerUserGames(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.OwnerUserGames[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.OwnerUserGames[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpOwnerUserGames(t *testing.T) {
	var err error
	...
	err = a.AddOwnerUserGames(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveOwnerUserGames(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.OwnerUserGames) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.OwnerUserGames[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.OwnerUserGames[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Username`: `character varying`}
	_           = bytes.MinRead
)
